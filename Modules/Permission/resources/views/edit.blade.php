<x-layouts.app title="Update Permission">
    <div class="flex flex-col items-center justify-center min-h-screen">
    <h1 class="text-center font-bold text-2xl mb-5">Update Permission</h1>
    ...
    <div class="w-full max-w-md bg-neutral-primary-soft p-6 border border-default rounded-base shadow-xs">
        <form action="{{ route('permission.update', $permission->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="name" value="{{ $permission->name }}" id="name" name="name"
            ...
            <div class="mb-4">
                ...
                <select id="module" name="module" class="block w-full px-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-body">
                    ...
                    {{-- {{ $permission->module === $status->value ? 'selected' : '' }} --}}
                        <option value="{{ $status->value }}" @selected(old('module', $permission->module) === $status->value)>
                            {{ $status->value }}
                        </option>
                    @endforeach
                </select>
